...
## 6-1) i18n/로케일 관리 (modules/i18n)

- 다국어 리소스 로딩 및 번역 키 관리
- UI 언어 변경 시 설정과 동기화
...
## 16) 프록시 로그 DB (modules/proxy_db)

- 프록시 요청 로그를 SQLite에 저장/조회/삭제
- 모니터링 화면과 API 로그 리스트의 데이터 소스

## 17) 보안 DB (modules/security_db)

- **IP 접근 로그 저장**
- 블랙리스트/화이트리스트 관리
- 요청 통계 및 순위 계산

## 18) Tauri Commands 요약
