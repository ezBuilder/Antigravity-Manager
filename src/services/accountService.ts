...
export async function startCodexOAuthAndWait(
  accountName: string,
): Promise<CodexAccountInfo> {
  return await invoke("start_codex_oauth_and_wait", { accountName });
}

export async function listCodexAccounts(): Promise<CodexAccountInfo[]> {
...
export async function getCodexUsage(
  accountId: string,
): Promise<import("../types/account").CodexUsageInfo> {
  return await invoke("get_codex_usage", { accountId });
}

/** 모든 Codex 계정 사용량 일괄 조회 */
export async function getAllCodexUsage(): Promise<
  import("../types/account").CodexUsageInfo[]
> {
  return await invoke("refresh_all_codex_accounts_usage");
}

/** Codex 계정 1건 사용량 재조회 (토큰 갱신 포함) */
export async function refreshCodexAccount(
  accountId: string,
): Promise<import("../types/account").CodexUsageInfo> {
  return await invoke("refresh_codex_account", { accountId });
}

/** Codex 계정을 계정 관리 목록용 Account 형태로 변환 (provider: 'Codex') */
export function codexToAccountLike(
  c: CodexAccountInfo,
  codexUsage?: import("../types/account").CodexUsageInfo,
): Account {
  const created = c.created_at
    ? Math.floor(new Date(c.created_at).getTime() / 1000)
    : 0;
  const lastUsed = c.last_used_at
    ? Math.floor(new Date(c.last_used_at).getTime() / 1000)
    : created;
  return {
    id: c.id,
...
export async function toggleProxyStatus(
  accountId: string,
  enable: boolean,
  reason?: string,
): Promise<void> {
...
export async function bindDeviceProfile(
  accountId: string,
...
export async function listDeviceVersions(
  accountId: string,
): Promise<DeviceProfilesResponse> {
...
export async function bindDeviceProfileWithProfile(
  ...
  return await invoke("bind_device_profile_with_profile", {
    accountId,
    profile,
  });
}
